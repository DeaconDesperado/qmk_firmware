...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [MAC_FN] = LAYOUT(
        ...
        KC_END, _______, _______, RGB_TOG, RGB_MOD, RGB_RMOD,RGB_HUI, RGB_HUD, RGB_HUD, RGB_SPI, RGB_SPD, RGB_VAI, RGB_VAD, _______, _______, _______,
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (battery_level >= 67) {
        ...
        // Low: 1 red LED (now actually visible!)
    ...
    if (leader_key_held) {
        rgb_matrix_set_color(10, DIVINE_BLUE);
    }
    ...
    // Device indicator constants (from config.h)
